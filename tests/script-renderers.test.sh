...
# instant, so without an explicit ordering the result is undefined — this passed
# on macOS and failed on Linux purely by luck of the filesystem.
age_mission_by "$home/missions/m-canon" 600
touch_now "$home/missions/m-drift/status.json"
